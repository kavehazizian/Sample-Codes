...
int main()
{
    ...
    for (IloInt i = 0; i < A_eq.n_rows; i++)
    {
        ...
        for (IloInt j = 0; j < A_eq.n_cols; j++)
        {
            ...
            else if (j < m * t_x + c_U && j >= m * t_x)
            {
                ctr += A_eq(i, j) * U[j - m * t_x];
            }
            else if (j >= m * t_x + c_U && j < m * t_x + c_U + c_W)
            {
                ctr += A_eq(i, j) * W[j - m * t_x - c_U];
            }
            else
            {
                ctr += A_eq(i, j) * R[j - m * t_x - c_U - c_W];
            }
        }
    ...
    for (IloInt i = 0; i < A_ineq.n_rows; i++)
    {
        ...
        for (IloInt j = 0; j < A_ineq.n_cols; j++)
        {
            ...
            {
                ctr_ineq += A_ineq(i, j) * U[j - m * t_x];
            }
            else if (j >= m * t_x + c_U && j < m * t_x + c_U + c_W)
            {
                ctr_ineq += A_ineq(i, j) * W[j - m * t_x - c_U];
            }
            else
            {
                ctr_ineq += A_ineq(i, j) * R[j - m * t_x - c_U - c_W];
            }
        }
