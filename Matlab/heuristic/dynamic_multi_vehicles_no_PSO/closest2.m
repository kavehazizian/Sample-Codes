function [travelling_time,nearest_nodes,infeas_nodes,not_good_neghbor]=closest2(x_coord,y_coord,e_vec,l_vec,Max_ride,served_node_coord,served_nodes,current_time,speed,d_vec,infeas_list)
n=(size(e_vec,1)-2)/2;
...
for i=1:2*(n)
    if (ismember(i,served_nodes)==0)&&(ismember(i,infeas_list)==0)
       ...
       %%%%%%%%%%%%%%%%%%%%%Not good neighberhood calculation%%%
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       if i<n+1
           %%%From examining node to its destination%%%
     ...
     if feas_check_des<0||feas_check_des_des<0
       ...
       %%%%%%%%%%%%End of not good neghberhood cal.%%%%%%%%%%%%%%%%%%%%%
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       
       %%%%%%%%%%Core part to calculate the distance%%%%%%
       ...
       elseif l_vec(i+1,1)>=arrival_time&&i<n+1&&feas_check>=0
           distance(i_node,1)=i;
           distance(i_node,2)=alpha*dist+beta*max(waiting_time,0)+gamma*max(e_vec(i+n+1)-(arrival_time+max(waiting_time,0)+d_vec(i+1,1)+Max_ride),0);
