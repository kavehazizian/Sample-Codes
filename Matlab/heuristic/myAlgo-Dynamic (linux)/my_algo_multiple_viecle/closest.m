function [travelling_time,nearest_nodes,infeas_nodes,not_good_neghbor]=closest(x_coord,y_coord,e_vec,l_vec,Max_ride,served_node_coord,served_nodes,current_time,speed,d_vec,infeas_list,alpha)
n=(size(e_vec,1)-2)/2;
...
%             beta=1-(alpha);
           distance(i_node,1)=i;
           distance(i_node,2)=alpha(1)*dist/speed+alpha(2)*max(waiting_time,0)+alpha(3)*max(max(waiting_time,0)+dist/speed-Max_ride,0);%+20*gamm*max((-current_time-max(waiting_time,0)+l_vec(i+1,1))-Max_ride,0);
...
%              gamm=1-(alpha+beta);
           distance(i_node,1)=i;
           distance(i_node,2)=alpha(4)*dist/speed+alpha(5)*max(waiting_time,0)+alpha(6)*max(e_vec(i+n+1)-arrival_dest,0);
