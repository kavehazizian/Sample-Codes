...
function [ Total_distance_traveled,Total_dist_modified,served_nodes,start_time_mat,infeas_nodes_mat,load_vec,waiting_time_mat, ride_time]=final_result(alpha,x_coord,y_coord,d_vec, q_vec,e_vec, l_vec,n_s,m,max_route, Max_ride,v_cap,v_routing_time,v_speed,init_time)
global_time=0;
 pool_size=2*n_s+2;
 time_res=5;
    for i=1:m
    served_nodes{i}=[0];
    start_time_mat{i}=init_time(i);
    infeas_nodes_mat{i}=[];
     load_vec{i}=[0];
     current_time_mat{i}=init_time(i);
...
%       end
 ...
 for i_l=1:m
     ...
     still_onboard=[];
     for i_pass=i_rem_co:-1:ceil(i_rem_co/3)
