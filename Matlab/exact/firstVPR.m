...
%%%% Demands
Dem=.1*[18000,26000,11000,30000,21000]';

%%%%%Inequality constraints%%%%%%%%%%
A_ineq=zeros(nc-1+mc*nc*Num_vhiecles,Num_var);
%%%%Assignment
...
b_ineq=zeros(nc-1+mc*nc*Num_vhiecles,1);
b_ineq(1:nc-1,1)=-ones(nc-1,1);

%%%%% Equality Constraints%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A_eq=zeros(2*mc*Num_vhiecles+(mc-1)*Num_vhiecles+Num_vhiecles,Num_var);
b_eq=zeros(2*mc*Num_vhiecles+(mc-1)*Num_vhiecles+Num_vhiecles,1);
...
[x,fval,exitflag,output] = intlinprog(f_vec,intcon,A_ineq,b_ineq,A_eq,b_eq,lb,ub,options)
toc